...
`status_change` ログを時系列に再生し、単元ごとに:
...
**今回の対応: 係数・ロジックは変更していない。** 実運用 7〜14 日で次を観察してから Phase 3.0a で直す:

- 初日〜3 日目に数学が計画から消えるか（シナリオ B の再現）
- 数学に集中した翌日の計画が他科目だけになるか
- ユーザーが計画に従ったか / 無視したか（`plannedMinutes` 付き session の比率で測れる）

**[提案] 3.0a の修正方針（係数ではなく構造の修正）**

- 不足を「割合」ではなく **分数** で測る: `不足分 = 週目標分 × 配分率 × 経過日数/7 − 実績分`。総学習量が少ない間は不足も小さい
- 不足は **予算にだけ** 効かせ、優先度の +15 と強制枠との 3 重計上をやめる（どれか 1 か所）
- 主力科目には **最低予算を確保**（例: 残り時間 × 目標割合 × 0.8 は他科目が使えない）
- 強制枠は「その科目に履歴があり、かつ 7 日以上空いた」場合だけ（`null` を除外）。または運用開始から 7 日経過後
- 件数上限を「分数上限（予算 + slack）」に置き換え、数学は 1 ブロック 30 分 × 何件でも予算内なら入れる
- 受け入れ基準としてシナリオ A〜E をテスト化（例: B で数学 ≥ 40%、E で数学 ≥ 30% かつ他科目も 1 件以上）
...
| --------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------- | -------------------------------- |
| **2.6**（今回） | 本番運用準備、計測可能性確認、本設計書、データ保証テスト                                  | なし                                        | なし                             |
| **3.0a**        | 計画の配分補正の構造修正（§9）+ シナリオテスト。実運用データを根拠に実施                  | なし                                        | 理由表示の文言のみ               |
| **3.0b**        | 学習速度 + 残り作業量 + Deadline Risk を計算し、科目画面に **表示のみ**（シャドーモード） | なし                                        | 科目画面に数行                   |
| **3.0c**        | Deadline Risk を優先度に組み込み（係数 0 → 有効化）、計画の日内安定化（§8.3）             | なし                                        | 「済」表示・残り時間             |
