...
### 6.5 Phase 3.0b で実装した内容（実装済み・表示専用）

実装: `src/lib/forecast/`（`config.ts` / `learningSpeed.ts` / `pace.ts` / `buildForecast.ts` / `format.ts`）、
UI: `src/components/forecast/forecast-ui.tsx`（科目ページ「◯◯の見通し」、Dashboard「受験ペース」）。
**今日の計画には一切入力しない**（`buildStudyModel` / planner は無変更。`plan-regression.test.ts` で固定）。

§5・§6 の提案からの変更点:

| 項目                 | 提案（§5/§6）                    | 3.0b の実装                                                                                                                                                                         |
| -------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 学習速度の単位       | 科目 × ステージ別の分数          | **1単元を未学習→完了まで** の分数 1 つ。観測は「基本OK到達まで」の分数 ÷ そのステップの作業割合（`remainingWorkFactor` の差）                                                       |
| 推定                 | 科目ごとの中央値を prior に縮小  | 速度比（観測 ÷ prior、0.25〜3 に clamp）の **中央値** を、分野→科目→全科目の順に最初に件数が足りた階層で採用し、`n/(n+4)` で prior と混合。単元自身の実績があれば、さらにそれと混合 |
| 目標ステータス       | 志望校の重要度で 2 or 3          | 目標ステータスは持たず `remainingWorkFactor`（未1.0/中0.7/基0.35/定0.1/過0）で残作業を表す。Phase 3.1 で Coverage/Mastery に置換予定                                                |
| 週キャパシティ       | 直近 28 日                       | 直近 **14 日**（今日を除く完了日）。記録開始から 3 日未満は計画ペース（週目標 × 配分）で代用し「暫定」                                                                              |
| Deadline Risk        | 科目 pressure + 単元の最遅開始日 | **科目の pressure（必要ペース ÷ 最近のペース）のみ**。単元レベルは 3.0c 以降                                                                                                        |
| 単元の重み（weight） | 所要時間の倍率                   | 使わない（weight は重要度の意味で使われているため）                                                                                                                                 |

データの扱い:

- 速度の実績は `status_change` ログで **初めて基本OK以上になった時点より前に、その単元に学習記録がある** ものだけ。自己申告（記録なしでの変更）は除外。記録開始時点のステータス（例: オンボーディングで学習中）からの分だけを作業量として数える
- `statusLogs` をスナップショットに追加（`activityLogs` の `type` インデックスで読むだけ。DB スキーマ・Dexie version は不変）
- 対象単元はモデルと同じ（アーカイブ済み単元・非表示/アーカイブ科目は除外）。ロック中（前提未完了）の単元は、試験までに必要な作業なので **含める**
...
### 7.1 Phase 3.1 で実装した内容（実装済み・表示専用）

実装: `src/lib/learning-dimensions/`（`config.ts` / `examReadiness.ts` / `buildLearningDimensions.ts` / `format.ts`）、
UI: `src/components/learning-dimensions/dimensions-ui.tsx`。`buildLearningDimensions(model)` は forecast と同じく
StudyModel から読み取り専用で派生し、**planner・`buildStudyModel`・DB・migration は無変更**（`plan-regression.test.ts` で固定）。
対象範囲はモデルと同じ（アーカイブ済み単元・アーカイブ科目を除外。非表示科目は既存の習熟度・志望校準備度と同様に含める）。

**学習範囲（Coverage）** — 「一度はある程度学習した範囲の割合」。正答率ではない。

- 単元: `COVERAGE_CONFIG.statusCoverageFactor`（未 0 / 中 0.5 / 基・定・過 1.0）
- 分野・科目: `Topic.weight` による加重平均（既存の `weightedPercent` / `subjectScore` と同じ重み。UI 上も weight は「重要度」）
- ステータスのみで判定（記録があってもステータス未学習の単元は 0。§7 案の「session/result あり」は採用していない）

**習熟度（Mastery）** — 新しいロジックは作らない。単元は既存 `calculateMastery` の score、分野・科目は既存 `subjectScore`
（= `SubjectSummary.score` / `CategorySummary.score`）。未学習単元は 0 として範囲全体で平均するため、習熟度 ≤ 学習範囲 に近い値になる。
大学単位の習熟度は既存の志望校準備度（`universityReadiness`）と一致する（テスト済み）。

**本番準備度（Exam Readiness）** — 「保存された証拠から見た、本番形式の問題への準備指標」。合格可能性ではない。

```
単元:  baseline  = mastery × noExamEvidenceFactor(0.5)
       examShare = min(1, 本番形式の強さ / fullExamStrength(12), 記録数 × maxSharePerRecord(0.35))
       readiness = (1 − examShare) × baseline + examShare × 本番形式の得点率 − 模試失点ペナルティ
分野:  単元 readiness の weight 加重平均
科目:  (1 − paperShare) × 分野と同じ加重平均 + paperShare × 過去問・模試の総得点率
       paperShare = min(maxPaperShare 0.7, 強さ / fullPaperStrength 30, 枚数 × 0.3)
```

- 本番形式の証拠がない → 習熟度の 50%（0 にはしない。確信度は必ず「低」）
- 証拠 1 件（1 回分）では最大 35%、過去問 1 枚の総得点では科目の最大 30% までしか動かない
- 全体得点は単元ごとの証拠を完全には置き換えない（最大 70%）。ブラウザ確認で 3 枚＋模試 1 件が上限なしで 100% 置換になり、範囲 16% で準備度 78% になったため追加した

証拠の階層（`evidenceWeights`、1 問相当あたり）: 通常演習 0.2 < 発展 0.4 < 復習 0.5 < 過去問レベルの演習 0.6 < 模試 0.8 < 編入過去問 1.0。
通常演習・発展・復習は **既存の習熟度にすでに反映済み** なので、準備度では「証拠の強さ」（表示・比較用）にだけ数え、
準備度を動かすのは本番形式の段（`examTiers`: 過去問レベル・模試・過去問）だけ。

| データ                                | 扱い                                                                                                                                                    |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ExerciseResult`                      | type `past_exam` → 過去問、`mock` → 模試、difficulty `past_exam` → 過去問レベル、`review` → 復習、`advanced` → 発展、他 → 通常演習。1 件 最大 10 問相当 |
| `Review`（完了）                      | 復習（できた 1 / 怪しい 0.5 / できなかった 0）。強さのみ                                                                                                |
| `PastExamProblem`（単元付き）         | 単元の過去問証拠（得点/満点があれば得点率、なければ 正解 1 / 部分 0.5 / 不正解 0）。大問 1 つ = 3 問相当。同じ過去問の大問は 1 記録                     |
| `PastExam`（科目付き）                | 科目の総得点（正負両方の証拠）。年度数 = (大学 or ラベル, 年度) の種類                                                                                  |
| `MockExam.score/maxScore`（科目付き） | 科目の総得点（正負両方）                                                                                                                                |
| `MockExam.weakTopicIds`               | **負の証拠のみ**。挙がった単元から 1 回 5 点（新しさで減衰、上限 15）を引く。挙がっていない単元は何も変えない（記録なし ≠ 正解）                        |

- 新しさ（`recency`）: 30 日以内 1.0 / 90 日以内 0.6 / それ以前 0.3。既存の習熟度の減衰は「最後に勉強してからの日数」なので証拠の古さとは別物（二重にはならない）。`now` より後の日付の証拠は無視
- 証拠信頼度（`confidence`）: 高 = 最上位の段（面接以外は過去問、面接は模擬面接）の記録 3 件以上 **かつ** 2 年度以上（面接は不要）**かつ** 重みの 50% 以上が新しい / 中 = 本番形式の記録 2 件以上 / 他は低。過去問 0 件で高にはならない
- 科目タイプ: 面接は模擬面接（`ExerciseResult` type `mock` = 評価/満点）だけを本番形式とし、過去問・模試の総得点・単元の過去問は使わない。語学は問題演習型と同じ扱い（TOEIC 等の外部試験は `MockExam` に得点を記録すれば総得点として入る）
- 大学単位: 必要科目の重み（既存と同じ正規化）で 3 軸を加重平均。確信度は科目の低 0 / 中 1 / 高 2 を重みで平均（1.5 以上で高、0.75 以上で中）。必要でない科目は動かない（テスト済み）
- 既存の習熟度は面接でも過去問の結果を補正に使っている（既存の事実。3.1 では変更していない）

**Phase 3.0b forecast との関係**: `src/lib/forecast/masteryWork.ts` に習熟度ベースの連続的な残作業係数を追加
（習熟度の基準点 0/20/50/75/100 ↔ 現行のステータス係数 1/0.7/0.35/0.1/0 の区分線形。補正のない単元では現行と完全一致）。
**`buildForecast` には未接続**で、予測表示は 3.0b のまま。切り替えは実データで差分を見てから（3.0c 以降の提案）。

**表示**: 科目ページ「学力の3軸」（3 つの値・短い説明・確信度・折りたたみの根拠と注意書き）、分野行に「範囲 %」、
Dashboard の第一志望カードに 1 行、志望校カードに 1 行。「合格率」等の表現は使わない。

**実運用で見るべき値**: 過去問なしの科目で「習熟度の 50%」が実感と合うか / 過去問 1〜3 枚での上がり方（`maxSharePerRecord`・`maxSharePerPaper`・`maxPaperShare`）/
模試失点ペナルティの強さ / 確信度が「中」「高」になるまでに必要な記録数。

**今後**: 3.0c で Deadline Risk を計画に入れる際も、3 軸は計画に入れない（入れるなら別フェーズで検討）。
3.2 の週次レビューでは `buildLearningDimensions(snapshotAt(t))` で週初・週末の 3 軸を比較できる（証拠は `now` 以前だけを使う作り）。
...
| **3.0b**（済） | 学習速度 + 残り作業量 + Deadline Risk を計算し、科目画面に **表示のみ**（シャドーモード, §6.5） | なし                                        | 科目画面に数行                   |
| **3.0c**       | Deadline Risk を優先度に組み込み（係数 0 → 有効化）、計画の日内安定化（§8.3）                   | なし                                        | 「済」表示                       |
| **3.1**（済）  | Coverage / Mastery / Exam Readiness 分離表示（§7.1）                                            | なし                                        | 科目画面・志望校画面             |
